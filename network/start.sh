...
echo "crypto-config.yaml定义了一个排序节点组织（OrdererOrgs）和四个对等节点组织（PeerOrgs），包括Taobao、JD、Wenjin和RegCenter"
...
echo -e "\n\n\n"
echo "七、为 Wenjin（成都温江社区医疗中心）定义锚节点"
configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./config/WenjinAnchor.tx -channelID appchannel -asOrg Wenjin

echo -e "\n\n\n"
echo "八、为 RegCenter（监管中心）定义锚节点"
configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./config/RegCenterAnchor.tx -channelID appchannel -asOrg RegCenter
...
WenjinPeer1Cli="CORE_PEER_ADDRESS=peer1.wenjin.com:7051 CORE_PEER_LOCALMSPID=WenjinMSP CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/peer/wenjin.com/users/Admin@wenjin.com/msp"
...
RegCenterPeer1Cli="CORE_PEER_ADDRESS=peer1.regcenter.com:7051 CORE_PEER_LOCALMSPID=RegCenterMSP CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/peer/regcenter.com/users/Admin@regcenter.com/msp"
...
docker exec cli bash -c "$WenjinPeer0Cli peer channel join -b appchannel.block"
docker exec cli bash -c "$WenjinPeer1Cli peer channel join -b appchannel.block"
docker exec cli bash -c "$RegCenterPeer0Cli peer channel join -b appchannel.block"
docker exec cli bash -c "$RegCenterPeer1Cli peer channel join -b appchannel.block"
...
docker exec cli bash -c "$WenjinPeer0Cli peer channel update -o orderer.qq.com:7050 -c appchannel -f /etc/hyperledger/config/WenjinAnchor.tx"
docker exec cli bash -c "$RegCenterPeer0Cli peer channel update -o orderer.qq.com:7050 -c appchannel -f /etc/hyperledger/config/RegCenterAnchor.tx"
...
docker exec cli bash -c "$TaobaoPeer0Cli peer chaincode install -n fabric-mims -v 1.0.10 -l golang -p chaincode"
docker exec cli bash -c "$JDPeer0Cli peer chaincode install -n fabric-mims -v 1.0.10 -l golang -p chaincode"
docker exec cli bash -c "$WenjinPeer0Cli peer chaincode install -n fabric-mims -v 1.0.10 -l golang -p chaincode"
docker exec cli bash -c "$RegCenterPeer0Cli peer chaincode install -n fabric-mims -v 1.0.10 -l golang -p chaincode"
...
docker exec cli bash -c "$TaobaoPeer0Cli peer chaincode instantiate -o orderer.qq.com:7050 -C appchannel -n fabric-mims -l golang -v 1.0.10 -c '{\"Args\":[\"init\"]}' -P \"OR ('TaobaoMSP.member','JDMSP.member','WenjinMSP.member','RegCenterMSP.member')\""
...
echo -e "\n\n\n"
echo "十五、验证链码。在cli容器中进行链码交互，验证链码是否正确安装及区块链网络能否正常工作"
...
docker exec cli bash -c "$JDPeer0Cli peer chaincode invoke -C appchannel -n fabric-mims -c '{\"Args\":[\"hello\"]}'"
docker exec cli bash -c "$WenjinPeer0Cli peer chaincode invoke -C appchannel -n fabric-mims -c '{\"Args\":[\"hello\"]}'"

if docker exec cli bash -c "$RegCenterPeer0Cli peer chaincode invoke -C appchannel -n fabric-mims -c '{\"Args\":[\"hello\"]}'" 2>&1 | grep "Chaincode invoke successful"; then
  echo "[Successful] network 部署成功。后续如需暂时停止运行，可以执行 docker-compose stop 命令（数据已持久化保存在Docker Volume中，不会丢失数据）。"
  echo "✅ 协和医院（TaobaoMSP）已加入区块链网络"
  echo "✅ 301医院（JDMSP）已加入区块链网络"
  echo "✅ 成都温江社区医疗中心（WenjinMSP）已加入区块链网络"
  echo "✅ 监管中心（RegCenterMSP）已加入区块链网络"
  echo ""
  echo "🎉 四院联盟链部署成功！"
  exit 0
fi
