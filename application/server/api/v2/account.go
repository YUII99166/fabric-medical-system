...
func Register(c *gin.Context) {
	...
	// 调用智能合约记录到区块链（非阻塞：失败不影响数据库注册）
	var blockchainData map[string]interface{}
	resp, bcErr := bc.ChannelExecute("register", bodyBytes)
	if bcErr != nil {
		fmt.Printf("区块链注册失败(将仅注册到数据库): %v\n", bcErr)
	} else {
		if err := json.Unmarshal(bytes.NewBuffer(resp.Payload).Bytes(), &blockchainData); err != nil {
	...
	if err != nil {
		fmt.Printf("数据库注册失败: %v\n", err)
		appG.Response(http.StatusInternalServerError, fmt.Sprintf("注册失败: %v", err), nil)
		return
	}

	fmt.Printf("数据库注册成功，注册完成\n")
	...
	if bcErr != nil {
		...
		appG.Response(http.StatusOK, "成功", blockchainData)
	}
}
